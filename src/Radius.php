...
class Radius
{
    ...
    public function setAttribute($type, $value)
    {
        ...
        $multiAVP = [26, 79]; // vendor specific and EAP-Message
        if ($index > -1) {
            ...
            $this->attributesToSend[] = (in_array($type, $multiAVP)) ? [$temp] : $temp;
            $action = 'Added';
        }
    ...
    public function accessRequestList($serverList, $username = '', $password = '', $timeout = 0, $state = null)
    {
        ...
        if (! is_array($serverList)) {
            ...
            $this->errorMessage = sprintf(
                'server list passed to accessRequestList must be array; %s given',
                gettype($serverList)
            );
    ...
    public function accessRequestEapMsChapV2List($serverList, $username, $password)
    {
        ...
        if (! is_array($serverList)) {
            $this->errorCode = 127;
            $this->errorMessage = sprintf(
                'server list passed to accessRequestEapMsChapV2List must be array; %s given',
                gettype($serverList)
            );
     ...
     * @return bool|self false if the authenticator is invalid length, self otherwise
     */
